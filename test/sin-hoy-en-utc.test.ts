...
/** La raíz del paquete. `cwd()` en vitest es el directorio del proyecto. */
const RAIZ = cwd()
const DIRECTORIOS = [join(RAIZ, 'src')]
/** El paquete tiene ~35 módulos; si el barrido ve menos de 20, algo no anduvo. */
const MINIMO_DE_ARCHIVOS = 20

/** Las únicas rutas que pueden derivar un día de un `toISOString()`: en las dos
 *  el `Date` está anclado al mediodía UTC, así que el recorte no puede
 *  correrse. Es la excepción que hace que la regla se pueda cumplir en algún
 *  lado — y es una LISTA de rutas exactas, no un patrón. */
const EXENTAS = ['src/fechas.ts', 'src/agenda/fechas.ts']
