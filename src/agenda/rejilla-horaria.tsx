/** La rejilla horaria: las horas bajan por la izquierda y cada evento se
 ...
 *  `09:00 Corte` / `14:30 Color` dice *qué* hay ese día, pero no **cuánto
 *  ocupa** ni **dónde está el hueco**, que es la pregunta de quien agenda. Acá
 *  un evento de 3 horas mide el triple que uno de una, y el espacio en blanco
 *  entre las 11:00 y las 14:30 se ve como lo que es.
 *
 *  La usan dos de las tres vistas, con columnas distintas: la **semana** pone
 *  una columna por día, el **día** una por carril —cuadrilla, profesional,
 *  box— (el patrón de Google cuando mirás varios calendarios a la vez). El mes
 *  no la usa: un mes con rejilla horaria son 30 rejillas y no entra en ninguna
 *  pantalla.
 *
 *  ⚠️ **El solapamiento es la razón por la que este archivo no es tres divs.**
 *  Dos eventos a las 09:00 del mismo día se dibujarían uno encima del otro, y
 *  el de abajo desaparecería sin que nada avise. `colocar()` reparte el ancho.
 */
...
/** Alto de una hora, en píxeles. 56 px es lo que hace que un evento de media
 *  hora (28 px) todavía muestre su título en una línea. */
const ALTO_HORA = 56

/** Alto mínimo de un bloque. Un evento de 15 minutos son 14 px y no se puede
...
export type EventoRejilla = {
  ...
  detalle?: string
  /** Las clases de color del carril (ver `colores.ts`). */
  clase: string
          ...
          <div className="relative flex-1" style={{ height: alto }}>
            ...
            <div className="grid h-full" style={anchoColumnas}>
              {columnas.map((c) => (
                <div
                  key={c.clave}
                  // Anclaje estable de la columna: en una rejilla, "este evento
