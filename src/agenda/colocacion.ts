 ...
 *  los encuentra a los dos, y el evento de abajo simplemente no existe para
...
/** Reparte el ancho entre los eventos que se pisan.
 *
 ...
 *  las 9 de la mañana no adelgaza el evento de la tarde.
 */
export function colocar(eventos: EventoRejilla[]): Colocado[] {
  ...
  const cerrar = () => {
    ...
    const asignados = racimo.map((it) => {
      ...
      // 11:00), y a media columna cada uno la agenda diría que el carril
      // está doblemente ocupado cuando no lo está.
      //
  ...
  for (const it of items) {
    ...
    // 9 de la mañana no adelgace el evento de la tarde.
 ...
 *
 *  Google dibuja las 24 y scrollea. Acá la jornada entra en el horario
 *  laboral, y 24 horas dejarían dos tercios de la pantalla en blanco para
 *  mostrar lo mismo. Pero la ventana **se estira, nunca recorta**: un evento a
 *  las 05:00 baja el piso a las 5. Si recortara, ese evento desaparecería de la
 *  grilla sin que nada lo dijera, que es la peor forma de fallar de un
 *  calendario. */
