 ...
 *
 *  Contesta una pregunta distinta de la semana: no *"a qué hora entra esto"*
 *  sino *"cómo viene cargado el mes"* — dónde están los días llenos y dónde
 *  los vacíos, para planificar.
 *
 ...
 *  días del mes de al lado se muestran apagados en vez de dejarse en blanco:
 *  son días reales con eventos reales, y un lunes 31 de agosto en blanco porque
 *  septiembre empieza el martes escondería lo que hay ese día.
 */
import { Link } from 'react-router-dom'
import { cn } from '@/lib/utils'
import { NOMBRES_DIAS, mismoMes, sumarDias } from './fechas'
import { ChipEvento } from './chip'
import type { EventoRejilla } from './rejilla-horaria'

/** Cuántos eventos entran en una celda antes del "+N más". */
const TOPE = 3
        ...
        {dias.map((dia) => {
          ...
          return (
            ...
            >
              ...
              {eventos.slice(0, TOPE).map((e) => (
                <ChipEvento key={e.clave} evento={e} compacto />
              ))}
              {eventos.length > TOPE && (
                // Linkea al día, que es donde están los que no entraron. Un
                // "+2 más" que no lleve a ningún lado esconde eventos sin dar
                // forma de verlos.
                <Link
                  to={hrefDia(dia)}
                  className="px-1 text-xs text-muted-foreground hover:underline"
                >
                  +{eventos.length - TOPE} más
                </Link>
              )}
            </div>
          )
