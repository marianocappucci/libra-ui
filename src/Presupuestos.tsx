...
export function Presupuestos({ urlDelPdf = (id: number) => `/presupuestos/${id}/pdf` }: {
  ...
  async function load(busqueda = q) {
    ...
    setError(null)
    try {
      const data = await api.get<{ items: Presupuesto[]; counts: Record<string, number> }>(`/api/presupuestos?estado=${estadoFiltro}${busqueda ? `&q=${encodeURIComponent(busqueda)}` : ''}`)
    ...
    <div className="grid gap-4">
      ...
      <Card>
        <CardContent className="flex flex-wrap items-center gap-2 py-3">
          ...
          />
          <Button variant="outline" size="icon" onClick={() => load()} aria-label="Buscar"><Search /></Button>
          {q && <Button variant="outline" size="icon" onClick={() => { setQ(''); load('') }} aria-label="Limpiar búsqueda"><X /></Button>}
        </CardContent>
      </Card>
