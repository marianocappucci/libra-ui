/** Iconos de ACCIÓN y ESTADO de la familia Libra, en un solo lugar.
 *
 *  Vive acá y no copiado en cada producto porque es un **vocabulario**: la
 *  misma acción tiene que dibujarse igual en LibraDesk, Contalibra y el resto,
 *  y seis archivos que "tienen que decir lo mismo" terminan diciendo cosas
 *  distintas. La tabla completa —qué concepto lleva qué glifo— está en el wiki,
 *  en `estandares-desarrollo.md`, sección "Vocabulario canónico de acción y
 *  estado".
 *
 *  ## Lo que el producto consumidor tiene que tener
 *
 *  Este módulo importa `~icons/…`, que es un módulo **virtual**: lo resuelve
 *  `unplugin-icons` en compilación, en el pipeline del consumidor. O sea que el
 *  producto necesita, en su `frontend`:
 *
 *    - `unplugin-icons` + `@iconify-json/fluent` + `@iconify-json/fluent-color`
 *      + `@svgr/core` + `@svgr/plugin-jsx` como dependencias de desarrollo;
 *    - `Icons({ compiler: 'jsx', jsx: 'react' })` en los plugins de su
 *      `vite.config.ts`;
 *    - `"unplugin-icons/types/react"` en los `types` de su `tsconfig.app.json`.
 *
 *  No se puede declarar como `peerDependency`: npm no sabe mirar un
 *  `vite.config.ts`. Pero el que falte no se degrada en silencio — el build
 *  corta con `failed to resolve import "~icons/…"`, que dice exactamente qué
 *  falta. Y no es un requisito que agregue este módulo: el producto ya lo
 *  necesita para los iconos de IDENTIDAD del menú, que se le pasan a
 *  `createLayout` desde el producto, no desde acá.
 *
 *  ## El set
 *
...
// irse el recuadro. Se mantienen exportados para no romper a los consumidores
// que hoy los nombran (`ReporteDetalle` y `Configuracion` de LibraDesk); el día
// que esas pantallas se editen por otro motivo, se cambian por los canónicos.
