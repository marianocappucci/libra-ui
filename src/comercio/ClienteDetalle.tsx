// La ficha del cliente: datos y edición inline, la consulta del padrón, el
// auto-facturar de MercadoPago, los alias de facturación, la lista de precio del
// add-on mayorista, el resumen y las tablas de comprobantes, y la baja.
//
// Extraída de `pages/ClienteDetalle.tsx` de Contalibra y Restolibra (P9-M4,
// 2026-09-07). Lo que difería y cómo quedó:
// - `user.modulos.includes('mayorista')` (Contalibra) pasa a ser la prop
//   `conListaDePrecio`: el kit no sabe de sesiones ni de planes.
// - El botón "Eliminar cliente" al pie de la ficha era de Restolibra; queda para
//   los dos (el listado ya lo ofrecía por fila en ambos).
...
import { type AliasFacturacion, type ClienteConAlias, type ListaPrecio } from './tipos'
...
export function ClienteDetalle({ conListaDePrecio = false }: { conListaDePrecio?: boolean } = {}) {
  ...
  useEffect(() => {
    if (!conListaDePrecio || !clienteId) return
