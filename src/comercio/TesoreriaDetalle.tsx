// Una cuenta de tesorería: sus movimientos, la edición, el archivo y la transferencia.
//
// El "Limpiar" recargaba con el closure viejo (las fechas de antes de limpiar); ahora pasa las vacías.
//
// Extraída de `pages/TesoreriaDetalle.tsx` de Contalibra y Restolibra (P9-M4, 2026-09-07):
...
export function TesoreriaDetalle() {
  ...
  async function cargar(d = desde, h = hasta) {
    ...
    try {
      const data = await api.get<{ cuenta: CuentaTesoreria; movimientos: MovimientoTesoreria[] }>(
        `/api/tesoreria/cuentas/${cuentaId}?desde=${d}&hasta=${h}`,
      )
  ...
  function limpiarFiltros() {
    setDesde(''); setHasta('')
    // Con las fechas explícitas: el estado recién cambia en la próxima pintura
    // y `setTimeout(cargar, 0)` recargaba con las de antes.
    cargar('', '')
  }
