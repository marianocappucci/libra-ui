// Los movimientos de caja: listado por período y por caja, el resumen, el alta
// (con la caja y sus medios habilitados) y la anulación (P9-M3, 2026-09-06).
// Extraída de `pages/Caja.tsx` de Contalibra y Restolibra, idénticas salvo las
    ...
    <div className="grid gap-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="grid gap-2">
                  ...
                  <Select value={tipoMov} onValueChange={setTipoMov}>
                    <SelectTrigger id="mov-tipo" aria-label="Tipo"><SelectValue /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="grid gap-2"><Label htmlFor="mov-fecha">Fecha</Label><Input id="mov-fecha" type="date" value={fechaMov} onChange={(e) => setFechaMov(e.target.value)} /></div>
              </div>
              <div className="grid gap-2"><Label htmlFor="mov-concepto">Concepto</Label><Input id="mov-concepto" value={conceptoMov} onChange={(e) => setConceptoMov(e.target.value)} placeholder="Ej: Cobro factura cliente / Pago servicios" /></div>
              <div className="grid grid-cols-2 gap-3">
                <div className="grid gap-2"><Label htmlFor="mov-monto">Monto</Label><Input id="mov-monto" type="number" step="0.01" value={montoMov} onChange={(e) => setMontoMov(e.target.value)} /></div>
                <div className="grid gap-2"><Label htmlFor="mov-ref">Referencia</Label><Input id="mov-ref" value={referenciaMov} onChange={(e) => setReferenciaMov(e.target.value)} placeholder="Opcional — N° factura, proveedor, etc." /></div>
              </div>
              {cajas.length > 1 && (
                <div className="grid gap-2">
                  <Label>Caja</Label>
      ...
      <Card>
        <CardContent className="flex flex-wrap items-center gap-2 py-3">
          ...
          <Input type="date" aria-label="Desde" value={desde} onChange={(e) => setDesde(e.target.value)} className="w-40" />
          ...
          {(cajaId || desde !== primerDiaDelMesISO() || hasta !== hoyISO()) && (
            <Button variant="outline" size="icon" onClick={() => { setDesde(primerDiaDelMesISO()); setHasta(hoyISO()); setCajaId('') }} title="Limpiar filtros" aria-label="Limpiar filtros"><X /></Button>
          )}
        </CardContent>
      </Card>
