// El cierre de un turno: el arqueo contra el efectivo esperado (P9-M3,
// 2026-09-06). Byte a byte igual en los dos productos.
import { useEffect, useState } from 'react'
import { Link, useNavigate, useParams } from 'react-router-dom'
import { api, ApiError } from '../api-client'
import { formatoMoneda, type ResumenTurno, type Turno } from './tipos'
          ...
          <div className="grid gap-4">
                  ...
                  <>
                    {Object.entries(resumen.pagos_por_medio).map(([medio, total]) => (
                      <div key={medio} className="flex justify-between">
                        ...
                        <span className="font-medium">{formatoMoneda(total)}</span>
                      </div>
                    ))}
                    <div className="mt-1 flex justify-between border-t pt-1.5 font-semibold">
                      <span>Total recaudado</span><span>{formatoMoneda(resumen.total_ventas)}</span>
                    </div>
                  </>
                )}
              </CardContent>
            ...
            <Card className="border-amber-500/50">
              <CardContent className="grid gap-2 pt-6 text-sm">
                <div className="flex justify-between"><span className="text-muted-foreground">Efectivo en ventas</span><span>{formatoMoneda(resumen.efectivo_ventas)}</span></div>
                <div className="flex justify-between border-t pt-1.5 font-semibold"><span>Efectivo esperado en caja</span><span className="text-lg text-primary">{formatoMoneda(efectivoEsperado)}</span></div>
          ...
          <Card>
            ...
            <CardContent className="grid gap-3">
              ...
              <div className="grid gap-2">
                <Label htmlFor="cierre-monto">Efectivo contado en caja</Label>
                <Input id="cierre-monto" type="number" step="0.01" value={montoDeclarado} onChange={(e) => setMontoDeclarado(e.target.value)} className="text-lg" />
              </div>
              ...
              <div className="grid gap-2">
                <Label htmlFor="cierre-notas">Notas de cierre</Label>
                <Input id="cierre-notas" value={notas} onChange={(e) => setNotas(e.target.value)} placeholder="Ej: Todo en orden, faltante por error de vuelto…" />
              </div>
