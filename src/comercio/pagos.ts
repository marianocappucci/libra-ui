/** Las líneas de pago de un cobro: la forma y las cuentas, sin JSX (P9-M3).
 *
 *  Vino de `lib/pagos.ts` de Restolibra, que lo escribió para el POS y la cuenta
 *  del salón el 2026-08-31. Contalibra tenía en `Ventas.tsx` una versión
 *  anterior de lo mismo —filas que se autocompletaban con lo que faltaba— sin
 *  el vuelto: ésta la reemplaza, con el botón de *completar lo que falta* en el
 *  lugar del autocompletado.
 *
 *  Vive aparte de `LineasDePago.tsx` porque ese archivo exporta un componente
 *  y el fast refresh de Vite pide que un módulo exporte componentes **o**
 *  constantes, no las dos cosas. Además esto es lo que hay que poder probar
 *  sin montar nada.
 *
...
export type LineaDePago = {
   ...
   *  `libracommerce.web.ventas_router`. Sólo aplica al medio del QR. */
  cobrarConQr: boolean
